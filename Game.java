...
public class Game {
	...
	public static void main(String args[]) {
		...
		while(jumpin.state == GameState.RUNNING) {
	...
	public void testGameState(Game jumpin) {
		boolean allBunniesMoved = true;
		for(GamePiece g : jumpin.getPieces()) {
			if(g instanceof Bunny) {
				if(((Bunny) g).getHasMoved()){
					if(jumpin.getGameBoard().getTile(g.getX(), g.getY()).getGrass()) {
						jumpin.setGameState(GameState.LOST);
					}
				}
				else allBunniesMoved = false;
			}
		}
		if(allBunniesMoved) jumpin.setGameState(GameState.WON);
	}
}